...
const TopAiringPage: React.FC = () => {
  ...
  const loadTopAiringAnime = async (page: number) => {
    setLoading(true);
    try {
      const result = await fetchTopAnime(page);
      setAnimeList(result.data);
      setHasNextPage(result.pagination?.has_next_page || false);
    } catch (e) {
      toast({
        title: "Failed to fetch top airing anime.",
  ...
  useEffect(() => {
    loadTopAiringAnime(currentPage);
  }, [currentPage]);
    ...
    return (
      <div className="min-h-screen bg-[#0b1426] pt-20">
        <div className="max-w-7xl mx-auto px-3 sm:px-8 py-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Top Airing Anime</h1>
            <p className="text-[#9ca3af]">Discover the most popular currently airing anime series</p>
          </div>
